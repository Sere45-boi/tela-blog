...
export function DeleteArticleButton({ id, title }: { id: string; title: string }) {
  ...
  const modalContent = isOpen && (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-300" onClick={() => setIsOpen(false)}>
      <div 
        className="bg-white rounded-[2rem] p-10 max-w-sm w-full shadow-2xl border border-black/5 animate-in zoom-in-95 duration-300"
