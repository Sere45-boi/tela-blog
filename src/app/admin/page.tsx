...
export default async function AdminDashboard() {
  ...
  // Fetch all basic stats with safety limits to avoid memory crashes
  const [adsRes, analyticsRes, pageImpressionsRes, notificationsRes, allArticlesRes] = await Promise.all([
    ...
    supabase.from("analytics").select("read_time_seconds, article_id, reader_id, created_at").gt("created_at", last30Days).order('created_at', { ascending: false }).limit(2000),
    supabase.from("page_impressions").select("type, created_at").gt("created_at", last30Days).order('created_at', { ascending: false }).limit(3000),
